...
public interface PaymentRepo extends JpaRepository<Payment, Long> {
    ...
    @Query(value = """
    SELECT CONCAT(YEAR(p.create_date), '-W', DATEPART(ISO_WEEK, p.create_date)) AS week,
           SUM(p.amount) AS amount
    FROM payment p
    WHERE p.status = 'COMPLETED'
      AND YEAR(p.create_date) = YEAR(GETDATE())
    GROUP BY YEAR(p.create_date), DATEPART(ISO_WEEK, p.create_date)
    ORDER BY week
""", nativeQuery = true)
    ...
    @Query(value = """
    SELECT FORMAT(p.create_date, 'yyyy-MM') AS month, SUM(p.amount) AS amount
    FROM payment p
    WHERE p.status = 'COMPLETED'
      AND YEAR(p.create_date) = YEAR(GETDATE())
    GROUP BY FORMAT(p.create_date, 'yyyy-MM')
    ORDER BY month
""", nativeQuery = true)
    ...
    @Query(value = """
    SELECT YEAR(p.create_date) AS year, SUM(p.amount) AS amount
    FROM payment p
    WHERE p.status = 'COMPLETED'
    GROUP BY YEAR(p.create_date)
    ORDER BY year
""", nativeQuery = true)
    ...
    SELECT 
        CASE 
            WHEN MONTH(p.create_date) BETWEEN 1 AND 3 THEN 'Q1'
            WHEN MONTH(p.create_date) BETWEEN 4 AND 6 THEN 'Q2'
            WHEN MONTH(p.create_date) BETWEEN 7 AND 9 THEN 'Q3'
            WHEN MONTH(p.create_date) BETWEEN 10 AND 12 THEN 'Q4'
        END AS quarter,
